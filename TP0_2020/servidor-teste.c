...
void *client_handler(void* socket) {
  ...
  begintp();		//inicializa arquivo com usuarios e matriculas
  ...
  //TROCAR MSG BUFFER PARA OITO BYTES
  //SENHA> READ0MSG
  //LIST0MSG

	{
	  case 'N':	//Mensagem de cadastro (NEW_USER)
		sscanf(buffer, "%s %s %s", tipo , nome, matricula);
		bufferb = new_user(nome,matricula);
		break;

		case 'S':		//Mensagem de Envio (SEND)
		sscanf(buffer, "%s %s %s %s %[^\n]s", tipo , nome, matricula, book, message);
		bufferb = sendmsgtp(nome, matricula, book, message);
		break;

		case 'R':		//Mensagem de leitura (READ)
		sscanf(buffer, "%s %s %s %s", tipo , nome, matricula, book);
		bufferb = readmsgtp(nome, matricula, book);
		break;

		case 'L':		//Mensagem de listamento (LIST)
		sscanf(buffer, "%s %s %s", tipo , nome, matricula);
		bufferb = list(nome, matricula);
		break;
